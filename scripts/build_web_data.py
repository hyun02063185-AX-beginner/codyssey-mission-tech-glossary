...
def prose(text):
    """본문 절은 화면에서 평문으로 렌더링된다. 마크다운의 inline code 백틱이 글자로
    그대로 보이므로 걷어낸다. 원본 마크다운은 그대로 두어 되돌릴 수 있게 한다.
    코드 블록(`코드 예`)에는 적용하지 않는다 — 그쪽은 <pre> 로 나간다."""
    text = re.sub(r'`([^`\n]*)`', r'\1', text)
    return text.replace('`', '')  # 짝이 맞지 않는 백틱도 화면에는 글자로 보인다
def sections(path):
...
def main():
    ...
    for t in master:
        ...
        t.update({'termKo':t.pop('term_ko'),'termEn':t.pop('term_en'),'missionRefs':[dict(r,context=prose(r.get('context') or '')) for r in t.pop('mission_refs')],'relatedTerms':t.pop('related_terms'),'contentStatus':t.pop('content_status'),'hasDetailedContent':bool(s),'hasWebtoon':any(x['termId']==t['id'] for x in pilots),'summary':prose(s.get('한 줄 설명','')),'easyExplanation':prose(s.get('쉽게 설명하면','')),'technicalExplanation':prose(s.get('정확한 설명','')),'howItWorks':prose(s.get('동작 원리','')),'missionContext':prose(s.get('이 미션에서는 왜 필요한가','')),'codeExample':re.sub(r'^```[a-zA-Z]*\s*\n|\n```\s*$','',s.get('코드 예','')),'limitationsOrEdgeCases':prose(s.get('주의할 점 / 경계 조건','')),'detailRelatedTerms':related,'commonMisconceptions':prose(s.get('흔한 오해','')),'comparisons':prose(s.get('비슷한 개념과의 차이','')),'peerReviewQuestions':prose(s.get('동료평가 질문',''))})
    ...
    connection_ids=[item['id'] for item in connections['connections']]; assert len(connection_ids)==len(set(connection_ids)) and len(connection_ids)>0
    for connection in connections['connections']:
        assert connection['status']=='PUBLISHED' and all(connection.get(field) for field in ('title','type','question','summary','terms','sections','misconceptions','missionLinks'))
        assert len(connection['terms'])==len(set(connection['terms'])) and all(term_id in ids for term_id in connection['terms'])
        node_ids={node['id'] for node in connection['diagram']['nodes']}; assert len(node_ids)==len(connection['diagram']['nodes'])
        assert all((node.get('termId') in ids if node.get('termId') else node.get('label')) for node in connection['diagram']['nodes'])
        assert all(edge['from'] in node_ids and edge['to'] in node_ids and edge['label'] for edge in connection['diagram']['edges'])
    ...
    (OUT/'glossary.json').write_text(json.dumps(master,ensure_ascii=False),encoding='utf-8'); (OUT/'missions.json').write_text(json.dumps(mission_map,ensure_ascii=False),encoding='utf-8'); (OUT/'webtoons.json').write_text(json.dumps(pilots,ensure_ascii=False),encoding='utf-8'); (OUT/'openbook-main-m01.json').write_text(json.dumps(openbook,ensure_ascii=False),encoding='utf-8'); (OUT/'concept-connections.json').write_text(json.dumps(connections,ensure_ascii=False),encoding='utf-8'); (OUT/'map-registry.json').write_text(json.dumps({'schemaVersion':'1.0','maps':generated_maps},ensure_ascii=False),encoding='utf-8'); (OUT/'mission-map-routing.json').write_text(json.dumps(routing,ensure_ascii=False),encoding='utf-8')
