...
ROOT=Path(__file__).resolve().parents[1]; CUR=ROOT/'data/curated'; OUT=ROOT/'src/data/generated'; TERMS=ROOT/'content/terms'; WEB=ROOT/'content/webtoons'; IMAGE_DIR=ROOT/'public/webtoons'; OPENBOOK=ROOT/'content/peer-review/main-m01-openbook.yaml'; FRONTEND_MAP=ROOT/'data/knowledge-maps/frontend/knowledge-map.json'; M01_OVERLAY=ROOT/'data/knowledge-maps/frontend/overlays/main-m01.json'
def sections(path):
...
def main():
    ...
    openbook=json.loads(OPENBOOK.read_text(encoding='utf-8')); knowledge_map=json.loads(FRONTEND_MAP.read_text(encoding='utf-8')); m01_overlay=json.loads(M01_OVERLAY.read_text(encoding='utf-8')); ids={x['id'] for x in master}; assert len(ids)==len(master); assert all(r['source_status'] in {'direct','required','related'} for x in master for r in x['missionRefs']); assert all(x['termId'] in ids for x in pilots); assert all(x in ids for x in openbook['quick_terms']); assert all(x in ids for r in openbook['requirements'] for x in r['term_refs'])
    ...
    map_node_ids={node['id'] for node in knowledge_map['nodes']}; assert knowledge_map['mapId']=='frontend'; assert set(m01_overlay['nodeIds'])=={f'term:{term_id}' for term_id in openbook['quick_terms']}; assert set(m01_overlay['nodeIds']) <= map_node_ids; assert all(edge['from'] in map_node_ids and edge['to'] in map_node_ids for edge in knowledge_map['edges'])
    OUT.mkdir(parents=True,exist_ok=True); (OUT/'glossary.json').write_text(json.dumps(master,ensure_ascii=False), encoding='utf-8'); (OUT/'missions.json').write_text(json.dumps(mission_map,ensure_ascii=False), encoding='utf-8'); (OUT/'webtoons.json').write_text(json.dumps(pilots,ensure_ascii=False), encoding='utf-8'); (OUT/'openbook-main-m01.json').write_text(json.dumps(openbook,ensure_ascii=False), encoding='utf-8'); (OUT/'frontend-knowledge-map.json').write_text(json.dumps(knowledge_map,ensure_ascii=False), encoding='utf-8'); (OUT/'frontend-overlay-main-m01.json').write_text(json.dumps(m01_overlay,ensure_ascii=False), encoding='utf-8')
