...
def main():
    ...
    for p in sorted(WEB.iterdir()):
        if p.is_dir() and (p/'concept.md').exists():
            s=sections(p/'concept.md'); pilots.append({'termId':p.name,'title':(p/'concept.md').read_text().splitlines()[0].removesuffix(' 웹툰 콘셉트').removeprefix('# '),'goal':s.get('학습 목표','').removeprefix('- '),'hasImage':False})
    for t in master:
    ...
    OUT.mkdir(parents=True,exist_ok=True); (OUT/'glossary.json').write_text(json.dumps(master,ensure_ascii=False)); (OUT/'missions.json').write_text(json.dumps(mission_map,ensure_ascii=False)); (OUT/'webtoons.json').write_text(json.dumps(pilots,ensure_ascii=False))
    ids={x['id'] for x in master}; assert len(ids)==len(master); assert all(r['source_status'] in {'direct','required','related'} for x in master for r in x['missionRefs']); assert all(x['termId'] in ids for x in pilots)
